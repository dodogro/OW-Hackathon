...
transaction.df.m <- merge(transaction.df.2, promotion.df)

sales_volumes <- transaction.df.m %>% 
  select(TransactionDate, ActualSales, DistributionChannel, StoreKey, PromoMechanic) %>% 
  ...
  tabPanel("Exploratory Analysis",
           sidebarPanel(
             ...
             checkboxGroupInput(inputId = "Channel",
                         label = "Channel", 
                         selected = "Online",
                         choices = unique(sales_volumes$DistributionChannel)),
             
             # pickerInput(inputId = "StoreKey",
             #             label = "Store",
           ...
           mainPanel(
            tabsetPanel(
              tabPanel(title = "Aggregate Sales Volumes",
                       plotOutput(outputId = "SalesVolumes"),
              tabPanel(title = "Volmes by PromoMechanic",
...
server <- function(input, output){
  ...
  datasetInput <- reactive({
      ...
      filter(DistributionChannel == input$Channel) %>% 
