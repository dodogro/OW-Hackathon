...
head(holiday.df <- read.table(paste0(path,"Hackathon_HolidaysMY_vShared.csv"), 
                                       ...
                                       sep = ","))
```
Why and whihc holidays could be important? We should search each period and merge the information
...
server <- shinyServer(
  ...
  function(input,output){
    ...
    datasetInput <- reactive({
      sales_volumes %>% filter(TransactionDate == input$dataset)
    })
    ...
    output$Sales_Volumes <- renderPlot({
      ...
      dataset <- datasetInput()
      ggplot(dataset, aes(x = TransactionDate, y=Daily_Sum)) + geom_line()
