...
plot(promo.avg.duration, type = "b")
```

```{r Unknown promotions}
unknown.promotions <- promotion.df[promotion.df$PromoMechanic == "Unknown",]
view(unknown.promotions)
```
...
transaction.df$StoreKey <- gsub("Key", "", transaction.df$StoreKey)
transaction.df$StoreKey <- gsub("key ", "", transaction.df$StoreKey)
transaction.df$StoreKey <- gsub(" key ", "", transaction.df$StoreKey)
transaction.df$StoreKey <- gsub("  key ", "", transaction.df$StoreKey)
...
There are 23978 registered transactions without information that needs to be removed
...
# Removing 2012 transactions, possible mistakes but not addressable to 2021
...
transaction.promotion.df$StoreKey <- gsub("_Key", "",
                                           transaction.promotion.df$StoreKey)
transaction.promotion.df$StoreKey <- gsub("Key", "",
                                          transaction.promotion.df$StoreKey)
transaction.promotion.df$StoreKey <- gsub("key ", "",
                                          transaction.promotion.df$StoreKey)
transaction.promotion.df$StoreKey <- gsub(" key ", "",
...
Now it is possible to merge the subsets
```{r Merging 2021 transactions with all information}
# CPI
transactions2021 <- left_join(transactions2021, 
                              cpi.df,
                              by = c("TransactionDate" = "Date_daily"))
...
Now it is possible to merge the subsets
```{r Merging 2022 transactions with all information}
# CPI
transactions2022 <- left_join(transactions2022, 
                              cpi.df,
                              by = c("TransactionDate" = "Date_daily"))
...
We are unable to detect duplicate transactions since there is not a unique key for each transaction. Duplicates might be related to the same type of transaction occurred on the same day.

```{r Checking duplicate observation}
# Computationally expensive (it takes a long time to process)
trans.duplicates <- rbind(duplicated(transactions2020),
                          duplicated(transactions2021),
                          duplicated(transactions2022))
```

# Saving datasets
```{r}
save(transactions2020, file = "transactions2020.RData")
save(transactions2021, file = "transactions2021.RData")
save(transactions2022, file = "transactions2022.RData")

```

_________________________________________________________
# Creating Dataset for the Interactive Dashboard
...
save(sales_volumes, file = paste0(path,"MergedData.RData"))
```
