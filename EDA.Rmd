...
pacman::p_load(ggplot2, tidyverse, lubridate, packcircles, viridis, ggiraph,
               lattice, gcookbook, tidyr, timechange, ggstream, plyr)
...
mu <- ddply(HN, "FestivityFlag", summarise, grp.mean=mean(Sales))
# Add mean lines
S + geom_vline(data=mu, aes(xintercept=grp.mean, color=FestivityFlag),
             linetype="dashed")

# Populations are slightly different with different means. Is this difference statistically significant? 

t.test(Sales ~ FestivityFlag, data = HN) # The test shows that even at 10% confidence the difference between the two populations would be considered not statistically different from 0. 
...
t.test(Discounts ~ FestivityFlag, data = HN)
