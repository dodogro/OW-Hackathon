...
Trend <- as.data.frame(cbind(trendAS, DTS$CPI_Monthly, DTS$Period))
colnames(Trend) <- c("trend", "CPI_M", "Period")
Trend$Period <- as.factor(Trend$Period)
Trend$trend <- as.numeric(Trend$trend)
Trend$CPI_M <- as.numeric(Trend$CPI_M)

print(myplot <- ggplot(Trend, aes(x = Period, y = scale(trend), group = 1)) +
...
# Is there a trend?
summary(lmAS <- lm(trendAS ~ CPI_M, data = Trend))
...
HN <- Sales  %>% 
  group_by(TransactionDate, WeekendFlag, Festivity, CPI_daily, FestivityFlag) %>% 
  summarise(Sales = sum(ActualSales),
            Discounts = sum(SalesDiscount)) %>% 
