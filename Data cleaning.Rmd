...
sales_volumes <- transaction.df %>% 
  select(TransactionDate, ActualSales) %>% 
...
```{r Interactive Dashboard}
ui <- fluidPage(
  fluidRow(column(width = 2, "Inputs", style = "background-color = blue"),
           column(width = 10, "Plot", style = "background-color = green")),
  titlePanel(h2("Sales_Volumes", align = "center")),
  sidebarLayout(    
    sidebarPanel(

      sliderInput(inputId = "Time", label = "Dates:",
                   min = as.POSIXct("2020-01-01", format = "%Y-%m-%d", tz = ""),
                   max = as.POSIXct("2022-12-31", format = "%Y-%m-%d", tz = ""),
                   value = c(min(transaction.df$TransactionDate), 
                             max(transaction.df$TransactionDate)),
                   timeFormat="%Y-%m-%d",
                   timezone = "+0100")),
    mainPanel(
      plotOutput("Sales_Volumes"),
      )
    )
  )
...
server <- shinyServer(
  ...
  function(input,output){
    ...
    # output$summary <- renderPrint({
      ...
      # summary(dataset$Daily_Sum)
    # })

    # plot time series
    output$Sales_Volumes <- renderPlot({
      if (length(row.names(datasetInput())) == 0) {
        print("Values are not available") 
        }
      myplot <- ggplot(datasetInput(), aes(x = TransactionDate, y = Daily_Sum)) + geom_line()
      print(myplot)
    }, 
...
```{r UI-Creation}
# Header
header <- dashboardHeader(title = "OW-Hackatow-Dashboard")
...
# Dashboard
body <- dashboardBody(
  box(plotOutput("TimeSeries_Sales",width = 8)),
  box(sliderInput(inputId = "Time", label = "Dates:",
                  min = as.POSIXct("2020-01-01", format = "%Y-%m-%d", tz = ""),
                  max = as.POSIXct("2022-12-31", format = "%Y-%m-%d", tz = ""),
                  value = c(min(transaction.df$TransactionDate), 
                             max(transaction.df$TransactionDate)),
...
server <- function(input, output){
  ...
  output$TimeSeries_Sales <- renderPlot({
    myplot <- ggplot(datasetInput(), aes(x = TransactionDate, y = Daily_Sum)) + geom_line()
    print(myplot)
  })
}
