...
transaction.df.m <- merge(transaction.df.m, product.df)

sales_volumes <- transaction.df.m %>% 
  select(TransactionDate, ActualSales, DistributionChannel, SalesDiscount, ProductCategory_Lvl2, StoreKey, UnitVolume) %>% 
...
sales_volumes %>% 
      ...
      #filter(DistributionChannel %in% input$Channel) %>% 
      #filter(StoreKey %in% input$StoreID) %>% 
      filter(ProductCategory_Lvl2 %in% "AC") %>% 
      group_by(TransactionDate) %>% 
      summarise(Daily_Mean = mean(ActualSales),
                Daily_Sum = sum(ActualSales),
                Daily_Discount = -sum(SalesDiscount),
