...
Why some promotions don't have the correspondent informations? What can we do about them?

_________________________________________________________
# Interactive Dashboard
```{r Interactive Dashboard}
# Trial 
# For the sake of trying the dashboard I'll reduce the columns, thus the size of our data frame

transaction.df.s <- transaction.df[sample(length(transaction.df$TransactionDate),10000),c(1,4,5,7,8)]
store.df <- store.df[,c(1,2)]
promotion.df <- promotion.df[,c(1,2)]
transaction.promotion.df <- transaction.promotion.df[sample(length(transaction.promotion.df$TransactionDate),10000),c(3,4)] 

transaction.df.1 <- merge(transaction.df.s, store.df)
transaction.df.2 <- merge(transaction.df.1, transaction.promotion.df) # This merge increments the rows length by a factor of 10^5....we need to find a solution

transaction.df.m <- merge(transaction.df, store.df)

sales_volumes <- transaction.df.m %>% 
  select(TransactionDate, ActualSales, DistributionChannel, SalesDiscount) %>% 
           ...
           mainPanel(
            tabsetPanel(type = "tabs",
              tabPanel(#splitLayout(cellWidths = c("60%", "20%", "20%")),
                ...
                box(verbatimTextOutput(outputId = "Summary")),
                tags$p("Values of Interest"),
                box(verbatimTextOutput(outputId = "Totals"))),
              
              tabPanel(title = "Volumes by PromoMechanic",
...
server <- function(input, output){
  ...
  output$SalesVolumes <- renderPlot({
    if (length(row.names(datasetInput())) == 0) {print("Values are not available")}
    print(myplot <- ggplot(datasetInput(), aes(x = TransactionDate, y = Daily_Sum)) +
            geom_line(color = "deepskyblue4") + 
            #geom_line(aes(x = TransactionDate, 
             #             y = Daily_Discount*(mean(Daily_Sum)/mean(Daily_Discount))),
              #        color = "yellow4") +
            #scale_y_continuous(
            # Features of the first axis
            #name = "AggregateSales",
            # Add a second axis and specify its features
            #sec.axis = sec_axis(~.*mean(Daily_Sum)/mean(Daily_Discount), 
  ...
  output$Summary <- renderPrint({
    summary(datasetInput()[,2:3])
  
    })
  
   output$Totals <- renderPrint({
     df <- cbind(sum(datasetInput()$Daily_Sum),
                sum(datasetInput()$Daily_Discount))
     colnames(df) <- c("Aggregate Volumes", "Aggregate Discount")
     print(df)
   })
}
...
# Da RI-guardare bene...
    datasetInput2 <- reactive({
      ...
      filter(TransactionDate >= input$Time[1] & TransactionDate <= input$Time[2]) %>% 
      filter(DistributionChannel %in%  input$Channel) %>% 
    # filter(StoreKey == input$StoreKey) %>% 
    # group_by(PromoMechanic) %>% 
      summarise(Daily_Sum = sum(ActualSales))
    
    })
    output$PromotionMechanism <- renderPlot({
    if (length(row.names(datasetInput2())) == 0) {print("Values are not available")}
    myplot2 <- ggplot(datasetInput2(), aes(x=DistributionChannel, y=Daily_Sum, fill=PromoMechanic)) +
    geom_bar(stat="identity", position=position_dodge()) +
    geom_errorbar(aes(ymin=Daily_Sum-sd, ymax=Daily_Sum+sd), width=.2,
