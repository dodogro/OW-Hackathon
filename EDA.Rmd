...
pacman::p_load(ggplot2, tidyverse, lubridate, packcircles, viridis, ggiraph,
               lattice, gcookbook, tidyr, timechange)
```
...
summary(lm(ChangeInSales ~ ChangeInDiscount, data = SD))

```
```{r Is Inflation Relevant?}
# De-trending Sales
df.full <- left_join(df, holidays.df, by = "TransactionDate")
df.full <- left_join(df.full, cpi.df, by = "TransactionDate")

DTS <- df.full %>% 
  group_by(TransactionDate, DistributionChannel, WeekendFlag, Festivity, CPI_daily) %>% 
  summarise(Sales = sum(ActualSales),
            Discounts = sum(SalesDiscount)) %>% 
  mutate(DetrendedSales = Sales/CPI_daily) %>% 
...
summary(lm <- lm(Sales ~ Trend, data = DTS[DTS$DistributionChannel == "Physical",]))

# Cool Plot Missing...

summary(lmD <- lm(DetrendedSales ~ Trend, data = DTS[DTS$DistributionChannel == "Physical",])) # Slight smaller t-value w.r.t trend but always extremely significant...What does this means? That CPI explains slighly the trend but it is definetely what drives the upwards patter .
