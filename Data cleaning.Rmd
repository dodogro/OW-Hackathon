...
transaction.df.pr <- transaction.df.pr[-c(4,5,11,12,13,14)]
transaction.df <- merge(transaction.df.pr, promotion.df)

sales_volumes <- transaction.df.pr %>% 
  select(TransactionDate, ActualSales, DistributionChannel, SalesDiscount, ProductCategory_Lvl2, StoreKey, UnitVolume, ProductKey, PromoMechanic) %>% 
